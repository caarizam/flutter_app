...
class LoginSteps extends Given2WithWorld<String, String, FlutterWorld>{
  ...
  Future<void> executeStep(String user, String password) async {
    ...
    await FlutterDriverUtils.enterText(world.driver, emailFinder, user, timeout: timeout);

    SerializableFinder passwordFinder = find.byValueKey('password');
    await FlutterDriverUtils.enterText(world.driver, passwordFinder, password, timeout: timeout);

    SerializableFinder loginButtonFinder = find.byValueKey('login_button');
    await FlutterDriverUtils.tap(world.driver, loginButtonFinder, timeout: timeout);

    print('>> End Login action');
  }

  @override
  RegExp get pattern => RegExp(r'The client sign in with user {string} and password {string}');
}